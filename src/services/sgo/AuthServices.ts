...
class AuthServices {
  ...
  async signIn ({ email, password }: SignInParams): Promise<any> {
    try {
      const response = await this.httpClient.post('/api/autenticacao/autenticar', {
        data: { email, password }
      })
      return response
    } catch (error) {
  ...
  async validaToken ({ token }: ValidateTokenParams): Promise<any> {
    try {
      const response = await this.httpClient.get('/api/autenticacao/validatoken', {
        headers: {
          Authorization: token
        }
      })
      ...
      throw new Error('Falha ao validar o token.')
    }
  }
  ...
  async getUser ({ id, token }: GetUserParams) {
    try {
      ...
      throw new Error('Falha na busca por usuário. Por favor, verifique suas os dados.')
    }
  }
}
